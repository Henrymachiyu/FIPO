            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, "blue"):
                        ...
                        _logp_ppl = torch.exp(-1.0 * _logp_mean).item()
                        metrics.update({"actor/ppl": _logp_ppl})
                    ...
                    if self.config.algorithm.reweight.enable:
                        ...
                        elif self.config.algorithm.reweight.method in ['ppl', 'ppl-norm']:
                            # reweight by ppl
                            seq_mean_logp = (_old_logp * response_masks).sum(dim=-1) / response_masks.sum(dim=-1)
                            log_seq_ppl = - seq_mean_logp # log perplexity is -mean_logp
                            if self.config.algorithm.reweight.method == 'ppl-norm':
