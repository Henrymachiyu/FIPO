...
def compute_policy_loss_future_kl(
    ...
    #future_kl[i,j] = sum(negative_approx_kl[i,k] * response_mask[i,k] for k in range(j, response_length))
    ...
    # let's add a choice to define future mask to the e
    

    base_mask = torch.ones((response_len, response_len), dtype=torch.bool, device=device) # (response_length, response_length)
    if config.policy_loss.get("future_kl_start", "exclude_current") == "include_current":
        future_mask = torch.triu(base_mask, diagonal=0).expand(batch_size, -1, -1) # (batch_size, response_length, response_length)
    elif config.policy_loss.get("future_kl_start", "exclude_current") == "exclude_current":
        future_mask = torch.triu(base_mask, diagonal=1).expand(batch_size, -1, -1) # (batch_size, response_length, response_length)
    else:
        raise ValueError(f"Invalid future_kl_start: {config.policy_loss.get('future_kl_start', 'exclude_current')}")
    
    # let's set a sliding window for the future_mask
    if config.policy_loss.get("future_kl_window", -1) == -1:
        future_mask = future_mask
    # if the sliding window size is a number, we need to set the future_mask to the sliding window
    else:
        window_size = config.policy_loss.get("future_kl_window", -1)
        # 创建位置距离矩阵
        pos_i = torch.arange(response_len, device=device).unsqueeze(1)  # (response_len, 1)
        pos_j = torch.arange(response_len, device=device).unsqueeze(0)  # (1, response_len)
        distance = pos_j - pos_i  # (response_len, response_len)

        # 创建窗口限制mask
        window_limit = (distance >= 1) & (distance <= window_size)  # (response_len, response_len)

        # 将窗口限制应用到future_mask上
        future_mask = future_mask & window_limit.unsqueeze(0).expand(batch_size, -1, -1)
    
    
    #make future_mask also reflect the response_mask
    future_mask = future_mask & response_mask.unsqueeze(-1).to(torch.bool) # (batch_size, response_length, response_length)
    #calculate future_kl
    future_len = future_mask.sum(dim=-1).to(log_prob.dtype) # (batch_size, response_length)
    # 避免除零，确保数值稳定性
    future_len = torch.clamp(future_len, min=1.0)

    # add choice to directly use the log_prob sum as the future_kl
    if config.policy_loss.get("future_kl_use_log_prob_sum"):
        negative_approx_kl = log_prob


    if config.policy_loss.get("future_kl_average"):
        kl_expand = negative_approx_kl.unsqueeze(-1) * future_mask.to(log_prob.dtype) / future_len.unsqueeze(-1) # (batch_size, response_length, response_length)
    else: # sum the future_kl
        kl_expand = negative_approx_kl.unsqueeze(-1) * future_mask.to(log_prob.dtype) # (batch_size, response_length, response_length)
    # here we need to average the kl_expand along the response_length dimension
    future_kl = kl_expand.sum(dim=-1) # (batch_size, response_length)

    if config.policy_loss.get("future_kl_clip_high_only"):
        if config.policy_loss.get("future_kl_clip_ratio") != 0.0:
            clip_ratio = config.policy_loss.get("future_kl_clip_ratio")
            influence_weights = torch.clamp(torch.exp(future_kl), max=1+clip_ratio).detach()
        else:
            influence_weights = torch.clamp(torch.exp(future_kl),  max=10.0).detach()
    else:
        if config.policy_loss.get("future_kl_clip_ratio") != 0.0:
            clip_ratio = config.policy_loss.get("future_kl_clip_ratio")
            influence_weights = torch.clamp(torch.exp(future_kl),  min=1-clip_ratio,max=1+clip_ratio).detach()
        else:
            influence_weights = torch.clamp(torch.exp(future_kl),  max=10.0).detach()
    ...
    valid_future_ratio = torch.exp(torch.clamp(future_kl[response_mask.to(dtype=torch.bool, device=future_kl.device)], min=-20.0, max=20.0)).detach()
    valid_token_ratio = ratio[response_mask.to(dtype=torch.bool, device=negative_approx_kl.device)].detach()
    valid_future_ratio_mean = valid_future_ratio.mean()
    valid_future_ratio_min = valid_future_ratio.min()
    valid_future_ratio_max = valid_future_ratio.max()
    valid_token_ratio_mean = valid_token_ratio.mean()
    valid_token_ratio_min = valid_token_ratio.min()
    valid_token_ratio_max = valid_token_ratio.max()
    influence_weights_min = valid_vals.min()
    influence_weights_max = valid_vals.max()
    # future check the percentiles of the future_ratio and the token_ratio
    valid_future_ratio_50 = torch.quantile(valid_future_ratio, 0.5)
    valid_token_ratio_50 = torch.quantile(valid_token_ratio, 0.5)
    valid_future_ratio_75 = torch.quantile(valid_future_ratio, 0.75)
    valid_token_ratio_75 = torch.quantile(valid_token_ratio, 0.75)
    valid_future_ratio_90 = torch.quantile(valid_future_ratio, 0.90)
    valid_token_ratio_90 = torch.quantile(valid_token_ratio, 0.90)
    ...
    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, influence_weights_mean, influence_weights_min, influence_weights_max, \
        valid_token_ratio_min, valid_token_ratio_max, valid_future_ratio_50, valid_future_ratio_75, valid_future_ratio_90, \
        valid_token_ratio_50, valid_token_ratio_75, valid_token_ratio_90


    # return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, influence_weights_mean, influence_weights_min, influence_weights_max

# NOTE: kx add reweight loss
@register_policy_loss("reweight")
def compute_policy_loss_reweight(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    reweight_method = config.get("reweight_method", "none")  # reweight method, default is "none"
    reweight_c = config.get("reweight_c", 0.0)  # reweight coefficient, default is 0.0
    ...
    with torch.no_grad():
        if reweight_method == "none": pass
        elif reweight_method == "linear":
            cur_prob = torch.exp(log_prob.detach())
            weights = 1 + reweight_c - reweight_c * cur_prob
            advantages = advantages * weights
        elif reweight_method == "focal":
            cur_prob = torch.exp(log_prob.detach())
            weights = (1 - cur_prob) ** reweight_c
            advantages = advantages * weights
        else:
            raise NotImplementedError(f"Unsupported reweight method: {reweight_method}.")
